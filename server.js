...
// ✅ CORS FIX (make sure this comes BEFORE any route definitions)
app.use(cors({
...
// ✅ Manual CORS Headers (some setups still require this)
...
app.post("/userlocation", async (req, res) => {
  try {
    ...
    if (!email || !locationId)
      return res.status(400).json({ message: "Email and locationId required" });
    const result = await db.collection("userlocation").insertOne({ email, locationId, createdAt: new Date() });
...
app.get("/userlocation/:id", async (req, res) => {
  try {
    const { id } = req.params;
    if (!ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid ID" });
    const loc = await db.collection("userlocation").findOne({ _id: new ObjectId(id) });
...
app.put("/userlocation/:id", async (req, res) => {
  try {
    const { id } = req.params;
    if (!ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid ID" });
    const result = await db.collection("userlocation").updateOne(
      { _id: new ObjectId(id) },
      { $set: req.body }
    );
    if (result.matchedCount === 0) return res.status(404).json({ message: "Location not found" });
...
app.delete("/userlocation/:id", async (req, res) => {
  try {
    const { id } = req.params;
    if (!ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid ID" });
    const result = await db.collection("userlocation").deleteOne({ _id: new ObjectId(id) });
...
app.post("/reviews", async (req, res) => {
  try {
    ...
    const result = await db.collection("reviews").insertOne({ ...review, createdAt: new Date() });
...
app.post("/products", async (req, res) => {
  try {
    ...
    const result = await db.collection("products").insertOne({ ...product, createdAt: new Date() });
...
// REPLACE your GET /carts endpoint with the following:
...
app.post("/orders", async (req, res) => {
  try {
    ...
    const result = await db.collection("orders").insertOne({ ...order, createdAt: new Date() });
...
// In your backend file (index.js or app.js)
// REPLACE your GET /orders endpoint with the following:
app.get("/orders", async (req, res) => {
  try {
    if (!req.query.email) return res.status(400).json({ message: "Email required" });
    const orders = await db.collection("orders").find({ email: req.query.email }).toArray();
    res.json(orders);
  } catch (e) {
    res.status(500).json({ message: "Internal Server Error" });
  }
